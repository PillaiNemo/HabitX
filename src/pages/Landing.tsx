...
import { BarChart3, Target, CheckCircle2, Rocket, Calendar, Check, Lock } from 'lucide-react';
...
// A faint trail of grey/white dust motes following the cursor across the
// whole page — subtle and monochrome so it reads as texture, not decoration.
function useCursorTrail(canvasRef: React.RefObject<HTMLCanvasElement>) {
  useEffect(() => {
    ...
    interface Grain { x: number; y: number; vx: number; vy: number; life: number; maxLife: number; size: number; }
    const grains: Grain[] = [];
    let lastX = -1;
    let lastY = -1;
    ...
    const handleMove = (e: MouseEvent) => {
      ...
      const y = e.clientY;
      if (lastX >= 0) {
        // Spawn roughly in proportion to how far the cursor traveled this
        // event, capped, so a fast swipe doesn't dump dozens of grains at once.
        const dist = Math.hypot(x - lastX, y - lastY);
        const count = Math.min(2, Math.floor(dist / 20));
        for (let i = 0; i < count; i++) {
          if (grains.length > 70) grains.shift();
          grains.push({
    ...
    const frame = () => {
      ...
      for (let i = grains.length - 1; i >= 0; i--) {
        ...
        g.y += g.vy;
        g.vy *= 0.97;
        if (g.life >= g.maxLife) { grains.splice(i, 1); continue; }
        const t = g.life / g.maxLife;
        const alpha = (1 - t) * 0.3;
        ctx.beginPath();
        ctx.fillStyle = `rgba(225,225,232,${alpha})`;
...
// to scroll, on top of the section-level parallax.
...
export default function Landing() {
  ...
  // time is consistent, and pauses while the cursor is over the mock.
  useEffect(() => {
    if (cubePaused) return;
    const t = setInterval(() => goToFace((cube.face + 1) % FACES.length), 2000);
    return () => clearInterval(t);
  }, [cubePaused, cube.face]);
  ...
  // The CTA's background heatmap is purely decorative texture — fill it in
  // once it's actually likely to be visible rather than on initial mount.
  useEffect(() => {
    const el = document.querySelector('.cta-heatmap');
    if (!el) return;
    const io = new IntersectionObserver(entries => {
      if (entries[0].isIntersecting) { ctaHeatmapRef.current?.setProgress(1); io.disconnect(); }
    }, { threshold: 0.1 });
    io.observe(el);
    return () => io.disconnect();
  }, []);
  ...
  return (
    <div className="lp-root">
      <canvas ref={cursorTrailRef} className="cursor-trail-canvas" />
      ...
      <section className="final-cta reveal">
        ...
        <div className="cta-heatmap"><HeatmapField ref={ctaHeatmapRef} cols={40} rows={10} /></div>
        <div className="final-cta-inner">
          <div className="cta-stats">
            <div className="cta-stat"><strong>{Math.round(streakVal)}</strong><span>day streaks, easy</span></div>
            <div className="cta-stat-divider" />
            <div className="cta-stat"><strong>{Math.round(consistencyVal)}%</strong><span>weekly consistency</span></div>
            <div className="cta-stat-divider" />
            <div className="cta-stat"><strong>1</strong><span>tap to check in</span></div>
          </div>
          <h2>Your future self is built one day at a time.</h2>
          <p>Start your first streak in under a minute. No credit card required.</p>
          <button className="btn-hero" onClick={openSignup}>Get started free →</button>
        </div>
      </section>
